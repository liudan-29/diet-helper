...
- 核心任务：根据位置、餐次、用餐场合、同行人数、口味和预算，在一分钟内选出一家愿意去的附近餐厅。
- 数据来源：高德 MCP。
...
用户本次要解决的吃饭问题，包含位置、餐次、用餐场合、同行人数、预算和口味偏好。
...
| `occasion` | 是 | 日常、请客、约会、聚餐或用户填写的其他场合 |
| `occasionPreset` | 是 | 记录用户选择的预设项，自定义时为“其他” |
| `partySize` | 是 | 用户直接填写的同行人数，1至50人 |
| `budgetPerPerson` | 是 | 人均预算上限 |
| `searchRadius` | 是 | 初始搜索半径，默认 1 公里 |
| `tastePreferences` | 否 | 清淡、辣、少油、素食等可多选口味 |
| `customRequirement` | 否 | 用户补充的餐厅类型偏好或排除要求 |
| `createdAt` | 是 | 发起时间 |
...
3. 先按所选餐次的营业时间、场合和预算执行硬筛选，再按距离、评分和口味排序。
4. 浏览候选餐厅并在不足时补充。
