...
export async function searchAmapWebRestaurants(
  request,
  apiKey,
  fetchImpl = fetch
) {
  const result = await collectRestaurantCandidates(request, {
    queryPois: async ({ keywords, radius }) => {
      const endpoint = new URL("https://restapi.amap.com/v5/place/around");
      endpoint.searchParams.set("key", apiKey);
      endpoint.searchParams.set(
        "location",
        `${request.longitude},${request.latitude}`
      );
      endpoint.searchParams.set("keywords", keywords);
      endpoint.searchParams.set("types", "050000");
      endpoint.searchParams.set("radius", String(radius));
      endpoint.searchParams.set("sortrule", "distance");
      endpoint.searchParams.set("show_fields", "business,photos");
      endpoint.searchParams.set("page_size", "25");

      const response = await withTimeout(fetchImpl(endpoint), TIMEOUT_MS);
      if (!response.ok) throw new Error("高德Web服务周边搜索失败");
      const payload = await response.json();
      if (String(payload.status) !== "1") {
        throw new Error(
          `高德Web服务周边搜索失败：${String(payload.info || "未知错误")}`
        );
      }
...
export function selectRestaurantPhotos(restaurants, photoDetails) {
  ...
  return restaurants.map((restaurant) => {
    ...
    const mcpPhotos = (restaurant.photos || [])
      .map(normalizeAmapPhotoUrl)
      .filter(Boolean);
    const photos = webPhotos.length
      ? [mcpPhotos[0], ...webPhotos].filter(Boolean)
      : mcpPhotos;
    return {
      ...restaurant,
