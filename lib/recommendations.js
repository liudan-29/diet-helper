...
function inferBusinessStatus(hoursText) {
  const matches = String(hoursText).matchAll(/(\d{1,2}):(\d{2})-(\d{1,2}):(\d{2})/g);
  const ranges = Array.from(matches, (match) => ({
  ...
  const isOpen = ranges.some(({ start, end }) =>
    end >= start ? current >= start && current <= end : current >= start || current <= end
  );
