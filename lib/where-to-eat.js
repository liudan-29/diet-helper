...
export function isAmapDataMode(mode) {
  return String(mode || "").startsWith("amap");
}
...
export function createWhereToEat({ openRecordEditor, openModule }) {
  ...
  function showRestaurantDetails(event) {
    ...
    const content = `
      ...
      <p class="data-note">${isAmapDataMode(state.dataMode) ? "餐厅信息与实景照片来自高德，字段以本次查询快照为准。" : "当前为明确标注的演示数据。"}</p>`;
